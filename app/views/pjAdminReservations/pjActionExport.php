...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminReservations&amp;action=pjActionExport" method="post" id="frmExportReservations" class="form pj-form">
			...
			<span class="inline_block">
				<span class="block float_left overflow r20">
					<select name="period" id="export_period" class="pj-form-field w200 r20 float_left">
						<option value="all"<?php echo isset($_POST['period']) ? ($_POST['period'] == 'all' ? ' selected="selected"' : null) : ' selected="selected"'; ?> style="display:<?php echo isset($_POST['type']) ? ($_POST['type'] == 'file' ? ' block' : ' none' ) : ' block'; ?>;">-- <?php echo pjSanitize::html($export_periods['all']); ?> --</option>
						<option value="range"<?php echo isset($_POST['period']) ? ($_POST['period'] == 'range' ? ' selected="selected"' : null) : null; ?> style="display:<?php echo isset($_POST['type']) ? ($_POST['type'] == 'file' ? ' block' : ' none' ) : ' block'; ?>;"><?php echo pjSanitize::html($export_periods['range']); ?></option>
		...
		if(isset($_POST['type']) && $_POST['type'] == 'feed') 
		{
			...
			<div class="abFeedContainer">
				...
				<span class="inline_block">
					<textarea id="reservations_feed" name="reservations_feed" class="pj-form-field h80" style="width: 726px;"><?php echo PJ_INSTALL_URL; ?>index.php?controller=pjAdminReservations&amp;action=pjActionExportFeed&amp;format=<?php echo$_POST['format']; ?>&amp;calendar_id=<?php echo $_POST['calendar_id'];?>&amp;type=<?php echo $_POST['period']; ?>&amp;period=<?php echo $_POST['period'] == 'next' ? $_POST['coming_period'] : $_POST['made_period']; ?>&amp;p=<?php echo isset($tpl['password']) ? $tpl['password'] : null;?></textarea>
				</span>
			</div>
			<?php
		} 
	...
	myLabel.reservations = "<?php echo pjSanitize::clean(__('menuReservations', true)); ?>";
	myLabel.period = "<?php echo pjSanitize::clean(__('lblPeriod', true)); ?>";
	myLabel.calendar = "<?php echo pjSanitize::clean(__('lblCalendar', true)); ?>";
